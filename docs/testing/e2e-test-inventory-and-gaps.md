...
- `production-build-routing.spec.ts` runs against a local Vite production
  preview, verifies the typed web release manifest is emitted, and proves a
  `/dev/*` deep link resolves to the production not-found surface.
...
Dated read-only production observations no longer live inside this coverage
inventory. They are preserved with their original limits in
[Production Read Observations — July and August 2026](production-evidence/2026-08-production-read-observations.md).
Those observations are evidence for their recorded deployments, not living
route coverage or production mutation authority.
...
The authoritative living route list and production-test classifications are the
typed registry in `apps/web/src/routes/app-route-registry.ts`. Registry parity
tests require unique paths/IDs/smoke flows, explicit access, fixture, viewport,
and navigation-write metadata, and prove development harness routes are absent
from production builds. The human-readable list below is a compact view of that
checked registry.
