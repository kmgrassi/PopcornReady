...
<!-- agent-summary: Every completed task adds a concise structured entry to .agent/feedback/LOG.md. -->
...
Copy `.agent/worksheets/TEMPLATE.md` at task start, update it as evidence arrives, and commit it with the work. At least monthly, run an interactive feedback review: group entries by repeated failure, missing tool, unclear rule, and slow validation; select one small improvement; record that decision in a new worksheet.
