...
the pinned `story_blueprints.asset_id` pointer, while a storyboard-targeted
snapshot fences and moves the compatibility plan pointer. `story_panels` has
image/prompt references and selection state, but no semantic snapshot pointer,
