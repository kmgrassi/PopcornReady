...
create table public.catalog_entries (
  ...
  -- Requires the asset-embeddings migration (pgvector + fixed model/dims) — until
  -- that lands, ship full-text only and add this column then. vector(N) where N =
...
create index catalog_entries_search_idx on public.catalog_entries
  ...
  where status = 'published';
-- ANN index on the catalog-owned copied vector (added with the column, once the
-- asset-embeddings migration has enabled pgvector and fixed the dimension):
-- create index catalog_entries_embedding_idx on public.catalog_entries
--   using hnsw (search_embedding vector_cosine_ops) where status = 'published';
```
...
1. **PR1 — schema.** `catalog_entries`, enums (`catalog_entry_kind`,
   ...
   (public-read-published, owner-all-statuses). Include the catalog-owned
   `search_embedding`/`search_model`/`search_dims` columns + ANN index **iff**
   the asset-embeddings migration (pgvector + fixed dimension) has already
   landed; otherwise add them in a tiny follow-up once it does. Additive
   migration with a unique timestamp (verify against remote history per the
