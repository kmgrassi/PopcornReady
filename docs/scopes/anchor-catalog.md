...
1. **PR1 — schema.** `catalog_entries`, enums (`catalog_entry_kind`,
   ...
   (public-read-published, owner-all-statuses). Includes the catalog-owned
   `search_embedding extensions.vector(1536)` + `search_model`/`search_dims`
   columns (the asset-embeddings migration already enabled pgvector at dim
   1536); the ANN index is deferred to a later search PR to match
   `public.asset_embeddings`, which also ships without one. Additive migration
   with a unique timestamp (verify against remote history per the
