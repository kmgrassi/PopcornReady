...
export function dbFrom(deps?: CatalogDeps): SupabaseClient {
...
export async function embedCatalogSearchText(
  text: string,
  deps?: CatalogDeps
...
export function cursorOffset(cursor: string | null): number {
  if (!cursor) return 0;
...
export async function ownedCatalogEntryRow(
  db: SupabaseClient,
  entryId: string,
  publisherUserId: string
...
export async function publishedCatalogEntryRow(
  db: SupabaseClient,
  entryId: string
): Promise<CatalogEntryRow> {
  const row = await runQuery(
...
export async function targetProjectRow(
  db: SupabaseClient,
  workspaceId: string,
  projectId: string
