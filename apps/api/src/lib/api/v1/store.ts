...
import {
  addProjectScriptDraftWithDeps,
  getActiveProjectScriptDraftWithDeps,
  type ActiveProjectScriptDraft,
  type ScriptDraftStoreDeps,
} from "./store-script-draft";
import { getProjectWatchMedia } from "./store-asset-discovery";
import type { ProjectWatchMedia } from "./store-asset-discovery";
import { getProjectStoryboard, isAssetIdShape, requireProjectRow } from "./store-storyboard";
...
export type {
  ...
  V1Workspace,
  VisualAnchorPlan,
  VisualAnchorPlanItem,
  VisualAnchorPlanItemKind,
  WorkspaceRole,
} from "./store-types";
export type { ActiveProjectScriptDraft } from "./store-script-draft";
export type {
...
export async function dataAssetById(
  db: SupabaseClient,
  assetId: string
...
export async function selectedDataAsset(
  db: SupabaseClient,
  projectId: string,
  slotRole: string,
  kind: GraphAssetKind,
...
// Poster selection and generation workflow lives in ./store-poster.
import {
  ...
  type PosterGenerationContext,
  type PosterVisibilityOpts,
} from "./store-poster";
export {
  fillProjectPosterFromFirstFrame,
  findReusableGeneratedPoster,
...
export type { PosterGenerationAssetRef, PosterGenerationContext } from "./store-poster";
...
async function posterUrlFor(asset: Awaited<ReturnType<typeof projectPosterAsset>>): Promise<string | null> {
...
export async function mapProjectWithProjection(
  db: SupabaseClient,
  row: ProjectRow,
  opts: PosterVisibilityOpts = {}
): Promise<V1Project> {
...
export async function insertDataAsset(input: InsertDataAssetInput): Promise<DataAssetRow> {
...
export function assetMediaToKind(media: AssetMedia, kind: GraphAssetKind): AssetKind {
...
export async function mapAsset(row: AssetRow): Promise<V1Asset> {
...
export async function mapAssets(rows: AssetRow[]): Promise<V1Asset[]> {
...
} from "./store-storyboard";
export type {
  SaveStoryboardBeatInput,
  SaveStoryboardInput,
  SaveStoryboardSceneInput,
} from "./store-storyboard";
...
export interface AssetSemanticSearchRpcRow extends AssetRow {
  embedding_id: string;
  chunk_key: string;
  chunk_kind: string;
...
export interface AssetWithProjectRow extends AssetRow {
  projects?: {
    id: string;
    visibility: "public" | "private";
    status: "active" | "deleted";
...
export {
  ...
  searchPublicContent,
} from "./store-asset-discovery";
export type { DiscoverSearchItem, ProjectWatchMedia, WorkspaceAssetSummary } from "./store-asset-discovery";
...
const scriptDraftDeps: ScriptDraftStoreDeps = {
  getDb: getServiceSupabase,
  dataAssetById,
  insertDataAsset,
  createAction,
...
export function addProjectScriptDraft(
  input: Parameters<typeof addProjectScriptDraftWithDeps>[1]
): ReturnType<typeof addProjectScriptDraftWithDeps> {
  return addProjectScriptDraftWithDeps(scriptDraftDeps, input);
}

export function getActiveProjectScriptDraft(
  projectId: string
): Promise<ActiveProjectScriptDraft | null> {
  return getActiveProjectScriptDraftWithDeps(scriptDraftDeps, projectId);
}
