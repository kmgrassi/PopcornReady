...
export async function getAssetMediaUrls(
  ...
  if (!data) throw notFound(`Asset not found: ${assetId}`);
  const row = data as AssetRow & { lineage_id: string };

  // Resolve to the lineage HEAD. A regenerate inserts a NEW version (new id)
  // sharing this row's lineage_id, so the requested id may be superseded. Serve
  // the newest ready media version's bytes, so every reference to ANY version —
  // storyboard panels, the immutable action log behind the run-progress board,
  // the media viewer — renders the regenerated image without repointing.
  const head = await runQuery(
    "store.getAssetMediaUrls.head",
    db
      .from("assets")
      .select("*")
      .eq("workspace_id", workspaceId)
  ...
  return assetMediaUrlsForRow((head ?? row) as AssetRow);
}
...
// Regenerate an asset's media as a NEW VERSION in the same lineage. The asset
// graph is immutable — assets_guard_immutable rejects mutating params/version/
// content_hash on an existing row — so a re-run cannot swap bytes in place. It
// inserts a fresh row sharing `lineage_id` with version+1, pointing at the newly
// uploaded bytes, refreshed provenance, and a fresh content_hash/inputs_finger
// print so stale-detection sees the change. Read paths resolve a lineage to its
// head version (getAssetMediaUrls, listWorkspaceAssets), so every reference to
// any version renders the regenerated bytes without rewriting immutable rows.
// Returns the now-live media urls. Workspace-scoped to match the entry point.
export async function insertRegeneratedAssetVersion(
  workspaceId: string,
  assetId: string,
  ...
  const current = (await runQuery(
    "store.insertRegeneratedAssetVersion read",
    db
      .from("assets")
      .select("*")
      .eq("id", assetId)
      .eq("workspace_id", workspaceId)
      .maybeSingle()
  )) as (AssetRow & { lineage_id: string; version: number }) | null;
  if (!current) throw notFound(`Asset not found: ${assetId}`);

  const params = { schema_version: "asset_params.v1", provenance: update.provenance };
  const inputs = current.inputs ?? [];
  const now = new Date().toISOString();
  // Copy the lineage identity; bump version. Omit id/ref/slug so the insert
  // triggers assign a fresh uuid + agent-legible ref, and leave slug null — the
  // unique project handle stays on the originating version. Drop the prior
  // semantic_analysis: the new bytes invalidate it.
  const row: Record<string, unknown> = {
    ...
    params,
    inputs,
    inputs_fingerprint: inputsFingerprint(inputs, params),
    content_hash: update.contentHash ?? current.content_hash,
    created_by_action_id: current.created_by_action_id ?? null,
    filename: update.filename,
    remote_url: null,
    storage_key: update.storageKey,
    storage_bucket: update.storageBucket,
    source: current.source,
    duration_sec: update.durationSec ?? current.duration_sec,
  ...
  const data = await runQuery(
    "store.insertRegeneratedAssetVersion",
    db.from("assets").insert(row).select("*").single()
  );
...
export async function listWorkspaceAssets(
  ...
  // Collapse each lineage to its head (highest version): a regenerate inserts a
  // new version row sharing lineage_id, so without this the grid would show both
  // the superseded (dead-bytes) row and the live one as duplicate tiles.
  const headByLineage = new Map<string, WorkspaceAssetJoinRow>();
  for (const row of filtered) {
    const lineageRow = row as WorkspaceAssetJoinRow & { lineage_id?: string; version?: number };
    const key = lineageRow.lineage_id ?? row.id;
    const existing = headByLineage.get(key) as
      | (WorkspaceAssetJoinRow & { version?: number })
      | undefined;
    if (!existing || (lineageRow.version ?? 1) > (existing.version ?? 1)) {
      headByLineage.set(key, row);
    }
  }
  const heads = [...headByLineage.values()];
  const all: WorkspaceAssetSummary[] = heads.map((row) => {
