...
test("packet computes bounded downstream depth, pins canonical story pointers, and deduplicates targets", () => {
  ...
  assert.deepEqual(result.pins.storySnapshots.map((pin) => pin.rowKind), [
    "story_blueprint", "story_scene", "story_beat",
  ]);
...
test("an existing explicit story target without a canonical pointer pin fails closed", () => {
  const graph = snapshot();
  ...
  assert.throws(() => buildRerunDecisionPacket({
    ...
    targets: [{ kind: "storyboard", projectId: "p", storyboardId: "story-1" }],
  }), /missing its canonical pointer pin/);
});
...
test("timeline targets are authorized only from canonical top-level segment identity", () => {
  const ids = canonicalTimelineItemIds({
    segments: [{
      id: "segment-1",
      clipId: "asset-should-not-authorize",
