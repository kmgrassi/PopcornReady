...
test("launcher routes video, script, and asset outcomes and preserves legacy asset status links", async ({ page }) => {
  await page.goto("/create");

  const fullVideo = page.getByRole("link", { name: "Start a full video" });
  const script = page.getByRole("link", { name: "Start a script" });
  ...
  await expect(script).toHaveAttribute("href", "/create/script");
...
test("three Asset Studio choices remain keyboard-reachable without mobile overflow @mobile", async ({ page }) => {
